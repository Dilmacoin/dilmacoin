...
const string strMessageMagic = "DilmaCoin Signed Message:\n";
...
{
    // DilmaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Dilmacoin: 1 day
static const int64 nTargetSpacing = 60; //  Dilmacoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // DilmaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 60; // Dilmacoin: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // Dilmacoin: 1 day
...
{
    // DilmaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Ou voces param com essa zoeira ou eu corto o bolsa familia!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DilmaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DilmaCoinMiner : ProcessBlock, block not accepted");
    }
